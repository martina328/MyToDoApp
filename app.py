...
basedir = os.path.abspath(os.path.dirname(__file__))   # Gets the directory of the this file
todo_file = os.path.join(basedir, 'todo_list.txt')     # Creates the path to the to-do list file using the directory
...
async def recommend(id, refresh=False):
    ...
    if g.todo and not refresh:
        try:
            # Attempts to load any saved recommendation from the DB
    ...
    if refresh:
        ...
        # Converts list of links to a single string
